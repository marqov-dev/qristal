"""Overlay local adapter files on an existing CPU image and test offline.
...
name='qristal-adapter-'+uuid.uuid4().hex[:10]
...
try:
    ...
    for file in ['adapter.py','runtime.py','test_adapter.py','test_adapter_image.py']:
...
for test in ['test_adapter.py','test_adapter_image.py']:
