...
class MultipartTests(unittest.TestCase):
    def test_exact_production_identity(self):
        self.assertEqual(multipart.SHA256, 'c1b393fa0f5e33fd34764834704afb6e6f8151afe2ee5ac68df65bd20231b9b7')
        self.assertEqual(multipart.BYTES, 314255079)
    ...
    def test_timeout_uses_separate_exact_abort_child(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch.object(multipart, 'validate_payload'), patch.object(multipart.subprocess, 'run') as run:
                run.side_effect = [subprocess.TimeoutExpired('upload', 600), subprocess.CompletedProcess('abort', 0)]
                with self.assertRaises(RuntimeError):
