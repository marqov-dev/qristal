...
try:
 ...
 lines=out.decode().splitlines();reports=[x[len('QPP_WHEEL_AUDIT '):] for x in lines if x.startswith('QPP_WHEEL_AUDIT ')]
...
Path('/private/tmp/qpp-wheel-linkage-scipy-20260914.json').write_text(json.dumps(result,indent=2)+'\n')
