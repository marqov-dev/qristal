...
## Implemented source transformation
...
copy outside the inputs. It changes exactly one bound line in
...
The zero-context patch is a review artifact. The helper applies the equivalent
...
its effective byte manifest. Its changed file does not retain a pristine Git blob
identity; the effective manifest does not claim to be the original Git tree.
...
1. **Export version selection:** Core `CMakeLists.txt:16–28` calls `git describe`
   and fatals when exports have no `.git`. Add a separate reviewed transformation
   to consume an explicitly recorded upstream version derived from the locked
   commit's trusted metadata. Do not invent a tag or fabricate a Git checkout.
2. **CPM:** `cmake/add_dependency.cmake:4–14` downloads CPM0.36.0 when absent.
...
3. **Python:** preinstall the complete hash-locked Linux amd64/Python3.10 wheelhouse
...
4. **Fresh XACC:** supply its retained installed artifact and manifest, set
