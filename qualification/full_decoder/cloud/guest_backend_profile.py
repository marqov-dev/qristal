...
from instrument_sparse import instrument
...
try:
    ...
    sparse, sparse_patch, sparse_identity = instrument(pathlib.Path('/work/SparseStateVecAccelerator.cpp').read_bytes())
    report['sparse_profile_identity']=sparse_identity
    (OUT/'SparseStateVecAccelerator.profile.cpp').write_bytes(sparse)
    (OUT/'sparse-profile.patch').write_text(sparse_patch)
    stage('sparse-plugin-build',BASE+['-fopenmp','-fPIC','-shared','-DUS_BUNDLE_NAME=sparse_simulator_plugin_bundle',
        '/proof/out/SparseStateVecAccelerator.profile.cpp',
        '/work/build-core/sparse_simulator/cppmicroservices_resources.cpp',
        '/work/build-core/sparse_simulator/cppmicroservices_init.cpp']+INC+LIB+
        ['-o','/proof/out/libsparse_simulator.so.1.8.1'])
    stage('sparse-plugin-bundle',['/work/install-xacc/bin/usResourceCompiler4',
        '-b','/proof/out/libsparse_simulator.so.1.8.1','-z','/work/build-core/sparse_simulator/res_0.zip'],30)
    for destination in ['/work/install-core/lib/libsparse_simulator.so.1.8.1','/work/install-xacc/plugins/libsparse_simulator.so.1.8.1']:
        shutil.copyfile(OUT/'libsparse_simulator.so.1.8.1',destination)
    report['sparse_plugin_sha256']=hashlib.sha256((OUT/'libsparse_simulator.so.1.8.1').read_bytes()).hexdigest()
    ...
    report['loaded_sparse_libraries']={}
    for item in report['stages']:
        for line in item.get('stdout',item.get('partial_stdout','')).splitlines():
            if line=='LOADED_SPARSE_LIBRARY: /work/install-xacc/plugins/libsparse_simulator.so.1.8.1':
                path='/work/install-xacc/plugins/libsparse_simulator.so.1.8.1'
                report['loaded_sparse_libraries'][path]=hashlib.sha256(pathlib.Path(path).read_bytes()).hexdigest()
