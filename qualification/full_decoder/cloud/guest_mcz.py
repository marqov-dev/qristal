"""One bounded controlled-Z prototype qualification; installed libraries unchanged."""
...
report={'kind':'qb-mcz-isolated-cpu-vm','stages':[],
...
try:
    stage('mcz-build',['c++','-std=c++20','-O1','-DNDEBUG','/work/mcz_checks.cpp']+INC+LIB+['-o','/proof/out/mcz-checks'])
    for mode,marker in [('negative','PASS: 10 MCZ rejected inputs'),('qpp','PASS: 42 MCZ complex-state cases'),('sparse','PASS: 158 MCZ sparse interference cases')]:
        result=stage('mcz-'+mode,['/proof/out/mcz-checks',mode],60,60000)
